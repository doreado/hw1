...
function usrVal() {
  ...
  if (this.value.length < 4 || this.value.length > 16) {
    ...
    fetch("http://localhost/hw1/is_in_db.php?username=" + this.value)
...
function emailVal(event) {
  ...
  fetch("http://localhost/hw1/is_in_db.php?email=" + this.value)
    .then(response => response.json())
    .then(json => {
      if (json.there_is) {
        validationError(this.name, 'availability');
...
function subSignup(event) {
  ...
  if (!isValid()){
    ...
    fetch("http://localhost/hw1/get_credential.php", form_data)
      .then(response => response.json())
      .then(json => {
        if (json.success) {
